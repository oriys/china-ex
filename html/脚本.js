...
const 本地存储足迹版本钥匙 = 'china-ex-footprint-version';
const 足迹版本 = '2';
...
const 获取等级们并生效 = _=>{
    const 已保存等级们字串 = 本地存储.getItem(本地存储等级们钥匙);
    const 省元素们 = 获取所有省元素们();
    省元素们.forEach((元素,下标)=>{
        const 默认等级 = 默认等级们[元素[唯一标识]] || 零;
        const 当前等级 = 已保存等级们字串 === null
            ? 默认等级
            : 已保存等级们字串[下标] || 零;
        元素[设置属性](等级,当前等级)
    });
    if(本地存储.getItem(本地存储足迹版本钥匙) !== 足迹版本){
        省元素们.forEach(元素=>{
            const 新等级 = 默认等级们[元素[唯一标识]];
            if(新等级) 元素[设置属性](等级,新等级);
        });
        保存等级们();
        本地存储.setItem(本地存储足迹版本钥匙,足迹版本);
    }
};
添加事件监控(地区, 点击, 事件=>{
    事件[停止冒泡]();
